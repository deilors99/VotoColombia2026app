...
# ================= SIDEBAR - VOTACIÓN (corregida para accesibilidad) =================
with st.sidebar:
    st.header("🗳️ EMITE TU VOTO")
    with st.form("form_voto_sidebar"):
        nombre = st.text_input(
            label="✍️ Nombre completo",
            placeholder="Tu nombre",
            key="nombre_sidebar",
            help="Tu nombre completo (no se almacena públicamente)"
        )
        departamento = st.selectbox(
            label="📍 Departamento",
            options=["Selecciona...", "Antioquia", "Atlántico", "Bogotá D.C.", "Bolívar", "Boyacá", "Caldas", "Caquetá", "Cauca", "Cesar", "Córdoba", "Cundinamarca", "Huila", "La Guajira", "Magdalena", "Meta", "Nariño", "Norte de Santander", "Quindío", "Risaralda", "Santander", "Sucre", "Tolima", "Valle del Cauca"],
            key="depto_sidebar"
        )
        ult5 = st.text_input(
            label="🔢 Últimos 5 dígitos de cédula",
            max_chars=5,
            type="password",
            key="cedula_sidebar",
            help="Solo para evitar votos duplicados (anónimo)"
        )
        candidato = st.selectbox(
            label="🎯 Candidato de tu preferencia",
            options=candidatos,
            key="candidato_sidebar"
        )
